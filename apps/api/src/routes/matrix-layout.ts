...
const MODULE_KEY = 'matrix-layout';
...
const updateMatrixLayoutSchema = z.object({
...
function parseMatrixLayout(config: unknown): MatrixLayout | null {
  if (!config || typeof config !== 'object') return null;
  const obj = config as Record<string, unknown>;
  if (!obj.positions || typeof obj.positions !== 'object') return null;
  return config as MatrixLayout;
}

export const matrixLayoutRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // GET /spaces/:spaceId/matrix-layout
  fastify.get('/', async (request, reply) => {
    ...
    const layout = parseMatrixLayout(module?.config);
    return { positions: layout?.positions ?? {} };
  });

  // PUT /spaces/:spaceId/matrix-layout
  fastify.put('/', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const body = updateMatrixLayoutSchema.parse(request.body);
