...
- [x] BUG ItemEditModal : faux positif `hasChanges` sur tout item daté (guard « quitter sans sauvegarder » sans modification) — comparaison de dates state (heure locale, tronquée minute) vs `item.xxxDate` (ISO UTC + secondes) ; `toMinuteISO()` sur les deux côtés — 2026-09-07
